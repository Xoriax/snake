...
import { COMBO_WINDOW_MS, SHRINK_EVERY_MS } from "../app/config";
...
export function createState(canvas: HTMLCanvasElement, best: number, services: Services, opts: Options): GameState {
    ...
    const s: GameState = {
        ...
        dashEnabled: !!opts.dash, dashCooldownMs: (opts.dashCooldownSec ?? 5) * 1000, dashLeftMs: 0,
        combo: 1, comboTimerMs: 0,
        shrinkEnabled: !!opts.shrinkArena, shrinkInset: 0, shrinkTimerMs: 0,
        portalPhase: 0,
        particles: [],
        shakeMs: 0,
    ...
    if (s.obstaclesEnabled) for (let i = 0; i < (s.mode === "hardcore" ? 16 : 10); i++) s.obstacles.push(randomFreeCell(s));
...
export function randomFreeCell(s: GameState): Cell {
    ...
    do {
        ...
        !inBounds(c.x, c.y, s.grid, s.shrinkEnabled ? s.shrinkInset : 0)
    );
    return c;
}